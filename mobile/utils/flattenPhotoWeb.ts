 ...
 * 웹에서는 이 파일에서 브라우저 DOM의 <canvas> API를 직접 써서 크롭된 이미지와
 * 색감 필터(ctx.filter)를 한 장으로 합칩니다. RN Web은
...
/**
 * imageUri(이미 프레임 비율에 맞춰 잘리고 크기 조절된 이미지)에 필터를 입혀
 * base64(접두어 없음)로 반환합니다. size는 출력 정사각형 한 변의 픽셀
