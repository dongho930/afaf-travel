/**
 * 개인정보처리방침 본문. 원본은 docs/onestore/11_개인정보처리방침.md이고,
 * 원본을 고치면 이 파일도 같이 고쳐야 앱·웹 화면(app/privacy.tsx)에 반영됩니다.
 *
 * 문장 안의 **굵게** 표시는 화면에서 굵은 글씨로 그립니다.
 */
export type PolicyBlock =
  | { type: "paragraph"; text: string }
  | { type: "bullets"; items: string[] }
  // 표는 폰 폭에서 넘치지 않도록 행마다 카드로 그립니다. 첫 번째 열이 카드 제목입니다.
  | { type: "table"; headers: string[]; rows: string[][] };

export interface PolicySection {
  title: string;
  blocks: PolicyBlock[];
}

export const PRIVACY_POLICY_OPERATOR = "동행";
export const PRIVACY_POLICY_EFFECTIVE_DATE = "2026년 10월 3일";
/** 바로 전 방침의 시행일 — 방침 화면에 "이전 방침"으로 함께 적습니다. */
export const PRIVACY_POLICY_PREVIOUS_DATE = "2026년 9월 17일";

export const PRIVACY_POLICY_INTRO =
  `${PRIVACY_POLICY_OPERATOR}(이하 "운영자")은 경기포올 앱·웹 서비스(이하 "서비스")를 이용하는 분의 개인정보를 소중히 다루며, 「개인정보 보호법」 등 관련 법령을 지킵니다. 이 방침은 서비스가 어떤 정보를 왜 수집하고, 어떻게 보관·파기하는지 알려드립니다.`;

export const PRIVACY_POLICY_SECTIONS: PolicySection[] = [
  {
    title: "1. 수집하는 개인정보와 이용 목적",
    blocks: [
      {
        type: "table",
        headers: ["구분", "수집 항목", "이용 목적"],
        rows: [
          ["회원가입·로그인", "이메일 주소, 비밀번호(암호화 저장), 아이디", "회원 식별, 로그인, 본인 확인"],
          ["프로필 (선택)", "프로필 사진", "프로필 표시"],
          ["서비스 이용", "AI 여행 코스 요청 문장, 선택한 이동 유형·지역·방문 예정일", "맞춤 여행 코스 생성"],
          [
            "추천 품질 개선",
            "AI 추천 요청 문장, 선택한 이동 유형·지역·방문 예정일, 추천받은 장소와 그중 선택한 장소, 로그인한 경우 회원 식별번호",
            "추천이 실제로 도움이 되었는지 분석하고 추천 품질 개선",
          ],
          [
            "서비스 이용 (회원)",
            "저장한 여행·코스, 방문 기록, 리뷰(별점·글·사진), 접근성 제보, 게시물·댓글",
            "여행 기록 관리, 이용자 간 정보 공유",
          ],
        ],
      },
      {
        type: "bullets",
        items: [
          "로그인 없이 이용하는 기능(여행지 탐색, 코스 만들기)에서는 이메일 같은 회원 정보를 받지 않습니다.",
          "리뷰·게시물·댓글과 작성자 아이디는 다른 이용자에게 공개됩니다.",
          "추천 품질 개선 기록은 로그인하지 않은 이용자도 남으며, 이때는 누구인지 알 수 있는 정보(이메일·아이디 등)를 함께 남기지 않습니다.",
        ],
      },
    ],
  },
  {
    title: "2. 기기 권한과 정보 처리",
    blocks: [
      {
        type: "table",
        headers: ["권한", "사용하는 때", "처리 방식"],
        rows: [
          [
            "위치 (선택)",
            "현재 위치에서 여행지까지 길찾기",
            "기기에서 위치를 읽어 카카오맵 길안내로 넘기며, 운영자 서버에는 전송·저장하지 않습니다.",
          ],
          [
            "마이크·음성 인식 (선택)",
            "음성으로 여행 요청 입력",
            "기기 또는 브라우저의 음성인식 기능(Android는 Google, iOS는 Apple, 웹은 이용 중인 브라우저가 제공)이 글자로 바꿉니다. 이 과정에서 음성이 해당 회사의 음성인식 서비스로 전송될 수 있으며, 운영자 서버로는 바뀐 글자만 전송되고 음성은 전송·저장하지 않습니다.",
          ],
          ["사진 (선택)", "프로필·리뷰·게시물 사진 첨부", "이용자가 고른 사진만 업로드합니다."],
        ],
      },
      { type: "paragraph", text: "권한을 허용하지 않아도 해당 기능 외의 서비스는 이용할 수 있습니다." },
    ],
  },
  {
    title: "3. 보유 및 이용 기간",
    blocks: [
      {
        type: "bullets",
        items: [
          "회원 정보와 작성한 기록은 **회원 탈퇴 시까지** 보관하고, 탈퇴하면 지체 없이 파기합니다.",
          "로그인하지 않고 요청한 AI 코스 요청 문장과 생성 결과는 서비스 품질 확인을 위해 1년 동안 보관한 뒤 파기합니다.",
          "추천 품질 개선 기록은 **수집일로부터 90일** 동안 보관한 뒤 자동으로 파기합니다.",
          "법령에 따라 보관해야 하는 정보는 해당 기간 동안 보관합니다.",
        ],
      },
    ],
  },
  {
    title: "4. 개인정보의 파기",
    blocks: [
      {
        type: "bullets",
        items: ["전자 파일은 복구할 수 없는 방법으로 삭제합니다.", "업로드한 사진은 저장소에서 삭제합니다."],
      },
    ],
  },
  {
    title: "5. 자동 수집 장치(쿠키 등)의 설치·운영 및 거부",
    blocks: [
      {
        type: "bullets",
        items: [
          "서비스는 쿠키를 사용하지 않습니다.",
          "대신 이용 편의를 위해 앱의 기기 저장소(웹은 브라우저의 로컬 저장소)에 로그인 상태 유지 정보, 선택한 이동 유형, 화면 테마, 사용법 안내 확인 여부, 최근에 만든 코스를 저장합니다.",
          "이 정보는 이용 중인 기기에만 저장되며, 로그인 상태 유지 정보는 로그인한 이용자를 확인할 때만 운영자 서버로 전송합니다.",
          "앱을 삭제하거나 브라우저의 사이트 데이터를 삭제하면 지워집니다. 이 경우 다시 로그인해야 하고 선택했던 설정이 초기화됩니다.",
        ],
      },
    ],
  },
  {
    title: "6. 개인정보 처리 위탁 및 국외 이전",
    blocks: [
      { type: "paragraph", text: "서비스 운영을 위해 아래 업체에 처리를 맡깁니다." },
      {
        type: "table",
        headers: ["수탁 업체", "맡기는 업무", "이전되는 항목", "이전 국가"],
        rows: [
          [
            "Supabase Inc.",
            "회원 인증, 데이터베이스·사진 저장",
            "1항의 회원 정보·이용 기록",
            "대한민국 (서울, ap-northeast-2)",
          ],
          ["Render Services, Inc.", "서버 운영", "서비스 이용 중 전송되는 정보", "싱가포르"],
          ["Groq, Inc.", "AI 여행 코스 생성", "AI 코스 요청 문장, 이동 유형·지역·방문 예정일", "미국"],
          ["Kakao Corp.", "지도 표시, 길안내 연결", "길찾기 출발지·도착지 좌표", "대한민국"],
        ],
      },
      {
        type: "bullets",
        items: [
          "이전 시기·방법: 서비스 이용 시점에 네트워크로 암호화하여 전송",
          "보유 기간: 위탁 계약 종료 또는 3항의 보유 기간까지",
          "국외 이전을 원하지 않으면 회원가입과 AI 코스 요청을 하지 않을 수 있으나, 이 경우 해당 기능을 이용할 수 없습니다.",
        ],
      },
      {
        type: "paragraph",
        text: "운영자는 이용자의 개인정보를 위 목적 외로 제3자에게 제공하지 않습니다. 단, 법령에 따른 요청이 있는 경우는 예외입니다.",
      },
    ],
  },
  {
    title: "7. 이용자의 권리와 행사 방법",
    blocks: [
      {
        type: "bullets",
        items: [
          "이용자는 언제든지 자신의 개인정보를 조회·수정하거나 삭제·처리정지를 요청할 수 있습니다.",
          "아이디·비밀번호·프로필 사진은 앱의 프로필 화면에서 직접 바꿀 수 있습니다.",
          "**회원 탈퇴**는 아래 개인정보 보호책임자 이메일로 가입한 이메일 주소와 함께 요청하면, 확인 후 지체 없이 처리합니다.",
        ],
      },
    ],
  },
  {
    title: "8. 안전성 확보 조치",
    blocks: [
      {
        type: "bullets",
        items: [
          "비밀번호는 암호화하여 저장합니다.",
          "모든 통신은 HTTPS로 암호화합니다.",
          "개인정보에 접근할 수 있는 사람을 운영 인원으로 제한합니다.",
        ],
      },
    ],
  },
  {
    title: "9. 개인정보 보호책임자",
    blocks: [
      {
        type: "table",
        headers: ["항목", "내용"],
        rows: [
          ["성명", "장동호"],
          ["이메일", "zakuv2@naver.com"],
        ],
      },
      { type: "paragraph", text: "개인정보 침해에 대한 상담이 필요하면 아래 기관에도 문의할 수 있습니다." },
      {
        type: "bullets",
        items: [
          "개인정보침해신고센터: privacy.kisa.or.kr / 국번 없이 118",
          "개인정보분쟁조정위원회: www.kopico.go.kr / 1833-6972",
        ],
      },
    ],
  },
  {
    title: "10. 방침의 변경",
    blocks: [{ type: "paragraph", text: "이 방침이 바뀌면 시행 7일 전부터 서비스 공지로 알립니다." }],
  },
];

/**
 * 회원가입 화면에서 '동의를 받기 전에' 보여주는 수집 항목 요약입니다.
 *
 * 개인정보보호법 제15조는 수집 전에 수집 항목·이용 목적·보유 기간과 '동의를
 * 거부할 권리'를 알리도록 정하고 있고, 원스토어 검증도 같은 이유로 명시적
 * 동의 절차(체크박스 등)를 요구합니다. 그래서 방침 전문으로 보내기만 하지 않고
 * 가입 화면에서 바로 읽을 수 있게 둡니다.
 *
 * 내용은 위 PRIVACY_POLICY_SECTIONS의 1번(수집하는 개인정보)과 3번(보유 기간)에서
 * '회원가입' 항목만 간추린 것입니다 — 두 곳이 어긋나면 방침 전문을 먼저 고치세요.
 * 프로필 사진·리뷰처럼 가입 이후에 이용자가 스스로 올리는 정보는, 그 시점에
 * 수집하는 것이므로 가입 동의 요약에 넣지 않습니다(수집 최소화).
 */
export const SIGNUP_CONSENT_ITEMS: { label: string; value: string }[] = [
  { label: "수집 항목", value: "이메일 주소, 비밀번호(암호화 저장), 아이디" },
  { label: "이용 목적", value: "회원 식별, 로그인, 본인 확인" },
  { label: "보유 기간", value: "회원 탈퇴 시까지 (탈퇴 시 지체 없이 파기)" },
];

/** 동의 거부 권리와 그때의 불이익 고지 (개인정보보호법 제15조 제2항). */
export const SIGNUP_CONSENT_REFUSAL_NOTICE =
  "동의를 거부하실 수 있으며, 이 경우 회원가입이 제한됩니다. 로그인 없이 이용할 수 있는 기능(여행지 탐색, 코스 만들기)은 그대로 이용하실 수 있습니다.";
